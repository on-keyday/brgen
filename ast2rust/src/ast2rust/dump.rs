...
pub fn parse_ast(ast: AST) -> Result<Rc<RefCell<Program>>, Error> {
    ...
    for raw_node in &ast.node {
        let node = match raw_node.node_type {
            ...
            NodeType::BlockExpr => Node::BlockExpr(Rc::new(RefCell::new(BlockExpr {
                expr_type: None,
                calls: Vec::new(),
                expr: None,
                loc: raw_node.loc.clone(),
            ...
            NodeType::Input => Node::Input(Rc::new(RefCell::new(Input {
                expr_type: None,
                loc: raw_node.loc.clone(),
            }))),
            NodeType::Output => Node::Output(Rc::new(RefCell::new(Output {
                expr_type: None,
                loc: raw_node.loc.clone(),
            }))),
            NodeType::Config => Node::Config(Rc::new(RefCell::new(Config {
                expr_type: None,
                loc: raw_node.loc.clone(),
            }))),
            ...
            NodeType::IndentScope => Node::IndentScope(Rc::new(RefCell::new(IndentScope {
                elements: Vec::new(),
                scope: None,
                loc: raw_node.loc.clone(),
            }))),
    ...
    for (i, raw_node) in ast.node.into_iter().enumerate() {
        match raw_node.node_type {
            NodeType::Program => {
                ...
                let body = match nodes.get(struct_type_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(struct_type_body as usize)),
                };
                let body = match body {
                    Node::StructType(node) => node,
                    x => return Err(Error::MismatchNodeType(x.into(), body.into())),
                };
                node.borrow_mut().struct_type = Some(body.clone());
                ...
                let body = match scopes.get(global_scope_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(global_scope_body as usize)),
                };
                node.borrow_mut().global_scope = Some(body.clone());
            }
            NodeType::Binary => {
                let node = nodes[i].clone();
                let node = match node {
                ...
                let body = match nodes.get(expr_type_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(expr_type_body as usize)),
                };
                node.borrow_mut().expr_type = Some(body.try_into()?);
                ...
                let body = match nodes.get(left_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(left_body as usize)),
                };
                node.borrow_mut().left = Some(body.try_into()?);
                ...
                let right_body = match right_body.as_u64() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(right_body.into(), JSONType::Number))
                    }
                };
                let body = match nodes.get(right_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(right_body as usize)),
                };
                node.borrow_mut().right = Some(body.try_into()?);
            }
            NodeType::Unary => {
                let node = nodes[i].clone();
                let node = match node {
                ...
                let body = match nodes.get(expr_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(expr_body as usize)),
                };
                node.borrow_mut().expr = Some(body.try_into()?);
            }
            NodeType::Cond => {
                let node = nodes[i].clone();
                let node = match node {
                ...
                let body = match nodes.get(cond_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(cond_body as usize)),
                };
                node.borrow_mut().cond = Some(body.try_into()?);
                ...
                let body = match nodes.get(then_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(then_body as usize)),
                };
                node.borrow_mut().then = Some(body.try_into()?);
                ...
                let body = match nodes.get(els_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(els_body as usize)),
                };
                node.borrow_mut().els = Some(body.try_into()?);
            }
            NodeType::Ident => {
                let node = nodes[i].clone();
                let node = match node {
                ...
                let body = match nodes.get(base_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(base_body as usize)),
                };
                node.borrow_mut().base = Some(body.clone());
                ...
                let scope_body = match scope_body.as_u64() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(scope_body.into(), JSONType::Number))
                    }
                };
                let body = match scopes.get(scope_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(scope_body as usize)),
                };
                node.borrow_mut().scope = Some(body.clone());
            }
            NodeType::Call => {
                let node = nodes[i].clone();
                let node = match node {
                ...
                let body = match nodes.get(callee_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(callee_body as usize)),
                };
                node.borrow_mut().callee = Some(body.try_into()?);
                ...
                let body = match nodes.get(raw_arguments_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(raw_arguments_body as usize)),
                };
                node.borrow_mut().raw_arguments = Some(body.try_into()?);
                ...
                for (i, link) in arguments_body.iter().enumerate() {
                    let link = match link.as_u64() {
                        Some(v) => v,
                        None => return Err(Error::MismatchJSONType(link.into(), JSONType::Number)),
                    };
                    ...
                    node.borrow_mut().arguments.push(body.try_into()?);
                }
            ...
            NodeType::If => {
                ...
                node.borrow_mut().then = Some(body.clone());
                let els_body = match raw_node.body.get("els") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "els")),
                };
                ...
                node.borrow_mut().els = Some(body.clone());
            }
            NodeType::MemberAccess => {
                ...
                let body = match nodes.get(target_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(target_body as usize)),
                };
                node.borrow_mut().target = Some(body.try_into()?);
            ...
            NodeType::Index => {
                ...
                let index_body = match index_body.as_u64() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(index_body.into(), JSONType::Number))
                    }
                };
                let body = match nodes.get(index_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(index_body as usize)),
                };
                node.borrow_mut().index = Some(body.try_into()?);
                let end_loc_body = match raw_node.body.get("end_loc") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "end_loc")),
                };
            ...
            NodeType::Match => {
                ...
                for (i, link) in branch_body.iter().enumerate() {
                    let link = match link.as_u64() {
                        Some(v) => v,
                        None => return Err(Error::MismatchJSONType(link.into(), JSONType::Number)),
                    };
                    ...
                    node.borrow_mut().branch.push(body.clone());
                }
                let scope_body = match raw_node.body.get("scope") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "scope")),
            ...
            NodeType::Range => {
                ...
                let start_body = match start_body.as_u64() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(start_body.into(), JSONType::Number))
                    }
                };
                let body = match nodes.get(start_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(start_body as usize)),
                };
                node.borrow_mut().start = Some(body.try_into()?);
                ...
                let body = match nodes.get(end_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(end_body as usize)),
                };
                node.borrow_mut().end = Some(body.try_into()?);
            }
            NodeType::TmpVar => {
                let node = nodes[i].clone();
                let node = match node {
            ...
            NodeType::BlockExpr => {
                ...
                let calls_body = match calls_body.as_array() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(calls_body.into(), JSONType::Array))
                    }
                };
                for (i, link) in calls_body.iter().enumerate() {
                    let link = match link.as_u64() {
                        Some(v) => v,
                        None => return Err(Error::MismatchJSONType(link.into(), JSONType::Number)),
                    };
                    ...
                    node.borrow_mut().calls.push(body.clone());
                }
                let expr_body = match raw_node.body.get("expr") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "expr")),
            ...
            NodeType::Import => {
                ...
                let body = match nodes.get(import_desc_body as usize) {
                    Some(v) => v,
                ...
                node.borrow_mut().import_desc = Some(body.clone());
            }
            NodeType::IntLiteral => {
                let node = nodes[i].clone();
                let node = match node {
            ...
            NodeType::Loop => {
                ...
                let body = match nodes.get(init_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(init_body as usize)),
                };
                node.borrow_mut().init = Some(body.try_into()?);
                let cond_body = match raw_node.body.get("cond") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "cond")),
                };
                ...
                let body = match nodes.get(step_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(step_body as usize)),
                };
                node.borrow_mut().step = Some(body.try_into()?);
                ...
                let body = match nodes.get(body_body as usize) {
                    Some(v) => v,
            ...
            NodeType::Assert => {
                ...
                node.borrow_mut().cond = Some(body.clone());
            }
            ...
            NodeType::Field => {
                ...
                let ident_body = match ident_body.as_u64() {
                    Some(v) => v,
                    None => {
                        return Err(Error::MismatchJSONType(ident_body.into(), JSONType::Number))
                    }
                };
                let body = match nodes.get(ident_body as usize) {
                    Some(v) => v,
                ...
                node.borrow_mut().ident = Some(body.clone());
                ...
                let body = match nodes.get(field_type_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(field_type_body as usize)),
                };
                node.borrow_mut().field_type = Some(body.try_into()?);
                let raw_arguments_body = match raw_node.body.get("raw_arguments") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "raw_arguments")),
                };
                ...
                let body = match nodes.get(belong_body as usize) {
                    Some(v) => v,
                ...
                node.borrow_mut().belong = Some(Rc::downgrade(&body));
            }
            NodeType::Format => {
                let node = nodes[i].clone();
                let node = match node {
            ...
            NodeType::Function => {
                ...
                for (i, link) in parameters_body.iter().enumerate() {
                    let link = match link.as_u64() {
                        Some(v) => v,
                        None => return Err(Error::MismatchJSONType(link.into(), JSONType::Number)),
                    };
                    ...
                    node.borrow_mut().parameters.push(body.clone());
                }
                ...
                let body = match nodes.get(return_type_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(return_type_body as usize)),
                };
                node.borrow_mut().return_type = Some(body.try_into()?);
                let belong_body = match raw_node.body.get("belong") {
                    Some(v) => v,
                    None => return Err(Error::MissingField(raw_node.node_type, "belong")),
                };
                ...
                let body = match nodes.get(func_type_body as usize) {
                    Some(v) => v,
                ...
                node.borrow_mut().func_type = Some(body.clone());
            }
            NodeType::IntType => {
                let node = nodes[i].clone();
                let node = match node {
            ...
            NodeType::IdentType => {
                ...
                node.borrow_mut().base = Some(Rc::downgrade(&body));
            }
            NodeType::IntLiteralType => {
                let node = nodes[i].clone();
                let node = match node {
            ...
            NodeType::ArrayType => {
                ...
                let body = match nodes.get(base_type_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(base_type_body as usize)),
                };
                node.borrow_mut().base_type = Some(body.try_into()?);
                ...
                let body = match nodes.get(length_body as usize) {
                    Some(v) => v,
                    None => return Err(Error::IndexOutOfBounds(length_body as usize)),
                };
                node.borrow_mut().length = Some(body.try_into()?);
            }
            NodeType::FunctionType => {
                ...
                for (i, link) in parameters_body.iter().enumerate() {
                    ...
                    node.borrow_mut().parameters.push(body.try_into()?);
                }
            }
            NodeType::StructType => {
                let node = nodes[i].clone();
                ...
                for (i, link) in fields_body.iter().enumerate() {
                    let link = match link.as_u64() {
                        Some(v) => v,
                        None => return Err(Error::MismatchJSONType(link.into(), JSONType::Number)),
                    };
                    ...
                    node.borrow_mut().fields.push(body.try_into()?);
                }
            }
            NodeType::UnionType => {
                let node = nodes[i].clone();
                ...
                for (i, link) in fields_body.iter().enumerate() {
                    ...
                    node.borrow_mut().fields.push(body.clone());
                }
            }
            NodeType::Cast => {
                let node = nodes[i].clone();
