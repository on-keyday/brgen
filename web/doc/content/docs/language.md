...
これは、`{}`を排除することで見た目的に見やすくすることと書き方が統一されることを意図している。
...
この言語は仕様を記述する目的でも使うことができる。
仕様として記述する場合自動生成はできないが仕様としては書けるものというものが存在する。
つまりエンコーダー/デコーダーが自動生成できるのは言語仕様のサブセットでありしたがって言語仕様から生成できないからと言ってその仕様が無効とは限らない。
...
(もちろん簡単なものについては[静的コード解析](https://ja.wikipedia.org/wiki/%E9%9D%99%E7%9A%84%E3%82%B3%E3%83%BC%E3%83%89%E8%A7%A3%E6%9E%90)を作り込めば検証可能であろうが、現時点では対応していない)
...
また、動的にデフォルトのエンディアンを変更する構文もある (詳しくは [ビルトイン関数](./builtin.md)を参照)
...
本言語では enum 型は C++の`enum class`程度の強さの型として定義されている。
Rust のように完全にその範囲以外を取らないというわけでもないがかといって C 言語のように暗黙的な変換をされるほどではないという扱いとなっている。また範囲内かどうかを判定するのには`x.is_defined`という形式が使用できる。デフォルトでは範囲内かどうかは保証しない。
`enum`は以下のように定義される。
...
また通常`format`の一部として使用する場合は以下のように基底型を指定する必要がある。
...
現状はフォーマット間で情報を受け渡す方法がこれしかないが将来的には引数のような形で受け渡せるように構文を拡張したいと考えている。
