...
#define TRY_SEPARATED(...) (try_joint_write<CodeWriter>(__VA_ARGS__))
    ...
    // Register an identifier modifier that suffixes reserved words:
    // `if (is_reserved(name)) name += suffix;`. The reserved-word set stays
    // with the language (ebm2cpp keywords, ebm2wuffs keywords+primitives);
    // only the suffix strategy is shared. Languages that escape instead of
    // suffixing (ebm2zig's @"...") keep their own modifier.
    void suffix_reserved_identifiers(auto&& ctx, auto is_reserved, std::string suffix = "_") {
        ctx.module().set_identifier_modifier(
            [is_reserved = std::move(is_reserved), suffix = std::move(suffix)](ebm::StatementRef, std::string& name) {
                if (is_reserved(name)) {
                    name += suffix;
                }
            });
    ...
    struct ByteArrayLiteral {
        size_t length = 0;  // declared ARRAY length (element count)
        std::string bytes;  // raw string-literal bytes
    };

    // Detect `const <name>: [u8; N] = "string-literal"`: a fixed-length UINT8
    // array variable initialized from a LITERAL_STRING. Languages whose string
    // literals don't type-check as byte arrays (rust &str, cpp const char[])
    // emit a bytes-array literal instead via config().byte_array_literal_wrapper.
    ebmgen::expected<std::optional<ByteArrayLiteral>> detect_byte_array_literal(auto&& vctx) {
        ...
        return std::optional(ByteArrayLiteral{static_cast<size_t>(length_p->value()), std::string(str_lit.body.data)});
    }
