...
DEFINE_VISITOR(Statement_FIELD_DECL_before) {
    ...
    // A tagged union becomes a struct holding every arm at once. Hike has no
    // union type, so the arms do not overlap in memory; which one is live is
    // decided by the same condition the decoder branched on, exactly as in the
    // .bgn source.
