...
複製をやめた結果として「導出ビューをどこに置くか」が問われ、その答えの候補が side table である、
...
#### そもそも写像にできない (作者証言 2026-08-11)

より根本的に、**分岐は順序が意味論**なので連想構造に置けない。

```
if x <= 3:      # 分岐 0
elif x <= 5:    # 分岐 1 の実効条件は !(x <= 3) && (x <= 5)
```

前の条件が後の条件の前提を狭めるため、並びを崩すと意味が変わる。順序を持たない map は
スキーマが許しても正しくない構造になる。`vector<UnionCandidate>` という形は
**この意味論の直接の反映**であって、表現力の制約への妥協ではない。

(AST スキーマの `Type` が `IsArray` / `IsPtr` / `IsWeak` / `IsOptional` しか持たず map を
表現できないのは事実だが [`src/tool/gen/gen.go:102`]、それは理由ではない。
仮に map が書けても使ってはいけない。)

#### parser が合成 Ident / Field を scope に push することについて

これは上の表に**入れない**。実害が観測されていないため。

parser はスコープを作りながら進むので、union field を導出したその場で同じスコープへ
push するのは局所的には一貫している。動いており、LSP からは補完候補に union field が
出るという意味でむしろ望ましい。「段階を分けるべき」という定石からの逸脱ではあるが、
それは**様式の差であって欠陥ではない** (作者確認、2026-08-11)。

nast で binder 側がやることになるのは、nast の parser がそもそもスコープを作らないからで、
「直した」のではなく形が違うだけである。
