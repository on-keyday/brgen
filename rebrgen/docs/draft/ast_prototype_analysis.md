...
### 言語側は戻り値ベース / モジュール化 / グローバル副作用なしの制約下に置く (作者証言 2026-08-10)

`.bgn` の意味論は、**戻り値ベースで、モジュール化されていて、グローバルな副作用が無い**という
制約の下で成立するように作る。生成器や実装がこれを破ること自体は構わない。
**言語側が少なくともその制約下で動く形になっていればよい**、という位置づけである。

これは拘束というより、言語設計の下限を決めている。たとえば `state` (F0014) が
隠れたグローバルではなく宣言された型で、`resolve_state_dependency` が format ごとに
`state_variables` を明示的に伝播させるのは、この制約と整合する形になっている。

なお `libs2j` の側は `out_callback` / `cout_color_mode` を thread_local グローバルで持っており
(§1-6)、この制約から外れている。上の言い方に従えば、それは許されるが望ましくはない、という扱い。
