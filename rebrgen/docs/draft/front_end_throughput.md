...
- `lexer::Token` は src2json と共有。`token` を読む箇所が全部影響を受ける
- ビューの寿命は入力バッファに縛られる。`Stream::take()` が外へ渡す経路と、
  JSON 出力がトークン本文を書く経路を確認する必要がある
- utf16 / utf32 入力では本文が変換後の文字列なので、そのままビューにはできない
  (今の `parse_one` は `TokenBuf` が `std::string` でないとき `utf::convert` する)
