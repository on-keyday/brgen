...
const char* ast2c_NodeType_to_string(ast2c_NodeType val) {
	...
	case AST2C_NODETYPE_IDENTITY: return "identity";
...
const char* ast2c_TokenTag_to_string(ast2c_TokenTag val) {
	...
	case AST2C_TOKENTAG_PARTIAL_INT_LITERAL: return "partial_int_literal";
	default: return NULL;
	}
}
...
int ast2c_TokenTag_from_string(const char* str, ast2c_TokenTag* out) {
	...
	if (strcmp(str, "partial_int_literal") == 0) {
		*out = AST2C_TOKENTAG_PARTIAL_INT_LITERAL;
		return 1;
	}
	return 0;
}
...
int ast2c_IoMethod_from_string(const char* str, ast2c_IoMethod* out) {
	...
	if (strcmp(str, "input_bit_offset") == 0) {
		*out = AST2C_IOMETHOD_INPUT_BIT_OFFSET;
		return 1;
	}
...
int ast2c_FieldArgument_parse(ast2c_Ast* ast,ast2c_FieldArgument* s,ast2c_json_handlers* h, void* obj) {
	...
	void* peek = h->object_get(h, obj_body, "peek");
	...
	if (!peek) { if(h->error) { h->error(h,peek, "ast2c_FieldArgument::peek is null"); } return 0; }
	if (!peek_value) { if(h->error) { h->error(h,peek_value, "ast2c_FieldArgument::peek_value is null"); } return 0; }
	if (!type_map) { if(h->error) { h->error(h,type_map, "ast2c_FieldArgument::type_map is null"); } return 0; }
	if (!metadata) { if(h->error) { h->error(h,metadata, "ast2c_FieldArgument::metadata is null"); } return 0; }
	if(!h->array_size(h, metadata,&s->metadata_size)) {
	...
	if(!h->number_get(h,peek_value,&s->peek_value)) {
		if(h->error) { h->error(h,peek_value, "failed to parse ast2c_FieldArgument::peek_value"); }
		goto error;
	}
	return 1;
error:
...
// returns 1 if succeed 0 if failed
int ast2c_Identity_parse(ast2c_Ast* ast,ast2c_Identity* s,ast2c_json_handlers* h, void* obj) {
	if (!ast||!s||!h||!obj) {
		if(h->error) { h->error(h,NULL, "invalid argument"); }
		return 0;
	}
	...
	void* constant_level = h->object_get(h, obj_body, "constant_level");
	void* expr = h->object_get(h, obj_body, "expr");
	if (!loc) { if(h->error) { h->error(h,loc, "ast2c_Identity::loc is null"); } return 0; }
	if (!expr_type) { if(h->error) { h->error(h,expr_type, "ast2c_Identity::expr_type is null"); } return 0; }
	if (!constant_level) { if(h->error) { h->error(h,constant_level, "ast2c_Identity::constant_level is null"); } return 0; }
	if (!expr) { if(h->error) { h->error(h,expr, "ast2c_Identity::expr is null"); } return 0; }
	if(!ast2c_Loc_parse(&s->loc,h,loc)) {
		if(h->error) { h->error(h,loc, "failed to parse ast2c_Identity::loc"); }
		goto error;
	}
	return 1;
error:
...
int ast2c_StrLiteral_parse(ast2c_Ast* ast,ast2c_StrLiteral* s,ast2c_json_handlers* h, void* obj) {
	...
	void* constant_level = h->object_get(h, obj_body, "constant_level");
	void* value = h->object_get(h, obj_body, "value");
	void* binary_value = h->object_get(h, obj_body, "binary_value");
	void* length = h->object_get(h, obj_body, "length");
	...
	if (!binary_value) { if(h->error) { h->error(h,binary_value, "ast2c_StrLiteral::binary_value is null"); } return 0; }
	...
	s->binary_value = h->string_get_alloc(h,binary_value);
	if (!s->binary_value) {
		if(h->error) { h->error(h,binary_value, "failed to parse ast2c_StrLiteral::binary_value"); }
		goto error;
	}
...
int ast2c_Field_parse(ast2c_Ast* ast,ast2c_Field* s,ast2c_json_handlers* h, void* obj) {
	...
	void* arguments = h->object_get(h, obj_body, "arguments");
	void* follow_comment = h->object_get(h, obj_body, "follow_comment");
	...
	if (!comment) { if(h->error) { h->error(h,comment, "ast2c_Field::comment is null"); } return 0; }
	...
	if (!follow_comment) { if(h->error) { h->error(h,follow_comment, "ast2c_Field::follow_comment is null"); } return 0; }
...
int ast2c_State_parse(ast2c_Ast* ast,ast2c_State* s,ast2c_json_handlers* h, void* obj) {
	...
	if (!comment) { if(h->error) { h->error(h,comment, "ast2c_State::comment is null"); } return 0; }
...
int ast2c_Enum_parse(ast2c_Ast* ast,ast2c_Enum* s,ast2c_json_handlers* h, void* obj) {
	...
	if (!comment) { if(h->error) { h->error(h,comment, "ast2c_Enum::comment is null"); } return 0; }
...
int ast2c_EnumMember_parse(ast2c_Ast* ast,ast2c_EnumMember* s,ast2c_json_handlers* h, void* obj) {
	...
	if (!comment) { if(h->error) { h->error(h,comment, "ast2c_EnumMember::comment is null"); } return 0; }
...
int ast2c_Function_parse(ast2c_Ast* ast,ast2c_Function* s,ast2c_json_handlers* h, void* obj) {
	...
	if (!comment) { if(h->error) { h->error(h,comment, "ast2c_Function::comment is null"); } return 0; }
