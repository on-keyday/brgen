...
"Hello World"
# type literal
u8 # primitive type literal
<[]u8> # array type literal
format A:
   ..
A # user defined type literal
```
...
equation(`==`) with range unlimited (`..`) always evaluated to true
...
value = match 1:
    ...
    3 => value
    .. => 0 # range unlimited means any other
```
...
```py
input.get(length,[type literal])
input.peek(length,[type literal]) # this is peek proxy object. peek should implement same methods and properties of input
...
```py
output.put(x) # write x as N bit integer into . if type of x is larger than N bit, generator implementor must insert size check
```
  ...
  "node": [
    ...
    },
    {
      ...
      "body": {
        ...
        "endian": "shared_ptr<expr>",
        "endian_value": "optional<uint>"
      }
    },
    {
    ...
    },
    {
      ...
      "one_of": ["builtin_function", "builtin_field", "builtin_object"]
    },
    {
      "node_type": "builtin_function",
      "base_node_type": ["member", "builtin_member", "stmt", "node"],
      "loc": "loc",
      "body": {
        "belong": "weak_ptr<member>",
        "belong_struct": "weak_ptr<struct_type>",
...
- 2024/01/25: update version: 0.0.0.d4
  - add note of type literal
  - add state for keyword
