...
func generateSingleDeepCopy(w *gen.Writer, def *gen.Struct) {
	w.Printf("template<class NodeM,class ScopeM,class Subst=NullSubst>\n")
	w.Printf("std::shared_ptr<%s> deep_copy(const std::shared_ptr<%s>& node,NodeM&& node_map,ScopeM&& scope_map,Subst&& subst=Subst{}){\n", def.Name, def.Name)
	w.Printf("if(!node){\n")
	...
	for _, field := range def.Fields {
		if field.Type.IsArray {
			...
			if field.Type.IsWeak {
				w.Printf("new_node->%s.push_back(deep_copy(i.lock(),std::forward<NodeM>(node_map),std::forward<ScopeM>(scope_map),std::forward<Subst>(subst)));\n", tag)
			} else {
				w.Printf("new_node->%s.push_back(deep_copy(i,std::forward<NodeM>(node_map),std::forward<ScopeM>(scope_map),std::forward<Subst>(subst)));\n", tag)
			}
			w.Printf("}\n")
		} else if field.Type.IsPtr || field.Type.IsInterface {
			...
			w.Printf("new_node->%s=deep_copy(%s,std::forward<NodeM>(node_map),std::forward<ScopeM>(scope_map),std::forward<Subst>(subst));\n", field.Tag, nodeAccess)
		} else {
...
func generateDeepCopy(w *gen.Writer, defs *gen.Defs) {
	w.Printf("template<class T,class NodeM,class ScopeM,class Subst=NullSubst>\n")
	w.Printf("std::shared_ptr<T> deep_copy(const std::shared_ptr<T>& node,NodeM&& node_map,ScopeM&& scope_map,Subst&& subst=Subst{});\n")
	w.Printf("\n")
	for _, v := range defs.Defs {
		switch def := v.(type) {
		case *gen.Interface:
			w.Printf("template<class NodeM,class ScopeM,class Subst=NullSubst>\n")
			w.Printf("std::shared_ptr<%s> deep_copy(const std::shared_ptr<%s>& node,NodeM&& node_map,ScopeM&& scope_map,Subst&& subst=Subst{}){\n", def.Name, def.Name)
			w.Printf("if(!node) return nullptr;\n")
			w.Printf("if(auto r = subst(node)) { return r; }\n")
			for _, derived := range def.Derived {
				found, ok := defs.Structs[derived]
				if !ok {
				...
				w.Printf("return deep_copy(ast::cast_to<%s>(node),std::forward<NodeM>(node_map),std::forward<ScopeM>(scope_map),std::forward<Subst>(subst));\n", found.Name)
				w.Printf("}\n")
			}
			w.Printf("return nullptr;\n")
			w.Printf("}\n")
...
func generateNullSubst(w *gen.Writer) {
	w.Printf("struct NullSubst{\n")
	w.Printf("template<class T>\n")
	w.Printf("constexpr std::shared_ptr<T> operator()(const std::shared_ptr<T>&) const { return nullptr; }\n")
	w.Printf("};\n")
}
