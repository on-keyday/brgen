...
namespace json2c {
    ...
    struct CPrimitive : CType {
        ...
        CPrimitive(std::string name)
            : CType(CTypeKind::primitive), name(name) {}
    ...
    struct CArray : CType {
        ...
        CArray(std::shared_ptr<CType> element_type, std::string length)
            : CType(CTypeKind::array), element_type(element_type), length(length) {}
    ...
    struct CPointer : CType {
        ...
        CPointer(std::shared_ptr<CType> element_type)
            : CType(CTypeKind::pointer), element_type(element_type) {}
    ...
    std::shared_ptr<CType> get_type(const std::shared_ptr<ast::Type>& typ) {
        if (auto int_ty = ast::as<ast::IntType>(typ)) {
            ...
            if (int_ty->is_common_supported) {
                return std::make_shared<CPrimitive>(brgen::concat(int_ty->is_signed ? "" : "u", "int", brgen::nums(bit), "_t"));
            }
            else if (bit < 64) {
                bit = brgen::ast::aligned_bit(bit);
                return std::make_shared<CPrimitive>(brgen::concat("uint", brgen::nums(bit), "_t"));
            }
        }
        if (auto enum_ty = ast::as<ast::EnumType>(typ)) {
            auto name = enum_ty->base.lock()->ident->ident;
            return std::make_shared<CPrimitive>(brgen::concat("enum ", name));
        }
        if (auto arr_ty = ast::as<ast::ArrayType>(typ)) {
            auto element_type = get_type(arr_ty->element_type);
            if (arr_ty->length_value) {
                return std::make_shared<CArray>(element_type, brgen::nums(*arr_ty->length_value));
            }
            else {
