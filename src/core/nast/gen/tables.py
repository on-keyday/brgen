...
def _emit_dense(w: Writer, table: str, node_t: str) -> None:
    w.write(
        ...
        "        return n;\n"
        "    }\n"
        "    // 入っているものを id 昇順に回す。表を丸ごと書き出す側 (wire 変換など) が要る。\n"
        "    void for_each_entry(auto&& f_) const {\n"
        "        for (std::uint32_t i = 1; i < present_.size(); i++) {\n"
        "            if (present_[i]) { f_(i, values_[i]); }\n"
        "        }\n"
        "    }\n"
...
def _emit_sparse(w: Writer, table: str, node_t: str) -> None:
    w.write(
        ...
        "    void for_each_entry(auto&& f_) const {\n"
        "        for (auto& e : entries_) { f_(e.node, e.value); }\n"
        "    }\n"
        "    void as_json(auto&& s) const {\n"
        "        auto obj_ = s.object();\n"
        "        std::string key;\n"
...
def _emit_flag(w: Writer, table: str, node_t: str) -> None:
    w.write(
        ...
        "    // flag は値を持たないので、キーだけ渡す。dense/sparse とは引数の数が違う。\n"
        "    void for_each_entry(auto&& f_) const {\n"
        "        for (auto id : nodes_) { f_(id); }\n"
        "    }\n"
        "    void as_json(auto&& s) const {\n"
        "        auto obj_ = s.object();\n"
        "        std::string key;\n"
