...
**構文解析そのものの約 395 ms が単独で最大。** 内訳を測った。

### トークンの読み直しは起きていない

```
consume()   143193 回   (トークン数 143251 とほぼ同じ)
backward()     114 回
shrink()      3182 回
```

各トークンは 1 回しか消費されていない。先読みで捨てる式が 38% あるのは
**式ノードを作っては捨てている**のであって、トークン列を舐め直しているのでは
ない。`make<T>` は全体の 4% なので、そこも主因ではない。

### 別 TU 呼び出しで 34 ms

`stream.cpp` と `parse.cpp` を 1 つの TU にまとめて測ると:

```
別 TU (現状)   parse alone 463.3 ms
1 TU           parse alone 429.7 ms   -33.6 ms (7%)
```

`eos` / `expect_token` / `maybe_parse` が `stream.cpp` 側にあり、
`maybe_parse()` だけで 2880403 回 (1 トークンあたり 20 回) 呼ばれている。

### 最大は演算子の線形探索

行ごとの実行回数を計装で取ると (`llvm-profdata` + `llvm-cov`)、
`parse.cpp:930-944` の `consume_op` が突出している。

```cpp
std::optional<lexer::Token> consume_op(size_t& i, auto& ops) {
    for (i = 0; i < ops.size(); i++) {
        if (auto t = s.consume_token(ops[i])) { return t; }
    }
    return std::nullopt;
}
```

`bin_layers` は **10 段 / 演算子 42 個**で、`parse_low()` は式の位置ごとに
各段の全演算子を順に試す。結果:

```
expect_token(string_view)              1252144 回  (8.7 回/token)
  うち一致                                36747 回  (2.9%)
  うち相手が punct/keyword ですらない    524850 回  (41.9%)
```

**4 割は識別子やリテラルに対して演算子リストを舐めている。**
1 回の試行は `consume_token -> peek_token -> expect_token -> eos -> maybe_parse`
の連鎖で、しかも別 TU。

手の入れ方は 2 段階で考えられる:

- **タグで足切り** — 演算子になり得るのは punct と一部 keyword だけ。
  `consume_op` の先頭で現在のトークンのタグを見れば 41.9% がその場で落ちる。
  変更は数行
- **トークンから引く** — 演算子文字列 -> (段, 添字) の表を引いて、現在の段に
  属するかだけ見る。42 回の試行が 1 回の参照になる

**タグ足切りは実装して測った。回数は減るが、時間は動かなかった。**

```
比較回数  1252144 -> 1018354  (-19%)   一致数は 36747 で不変
```

時間は交互実行 8 往復で:

```
足切りあり  平均 400.6 ms / 最小 384.4 ms
足切り無し  平均 401.1 ms / 最小 385.8 ms
```

差は無い。**この変更は入れていない。** `ops` が記号だけという不変条件を
新たに抱えるのに見合わない。

### 計測の注意 (このマシン)

最初は「463.3 -> 407.7 ms、-55.6 ms」と出た。**これは誤り。**
before を計装ビルド直後 (熱い) に、after を後で測った逐次比較で、
サーマルスロットリングによる周波数差 (2.61 GHz <-> 1.99 GHz、約 24%) を
測っていた。交互に測り直すと差は消えた。

このドキュメントの他の数字も、逐次で取ったものは同じ疑いがある。
**性能の A/B は交互実行で取ること。** 分解能は 10 ms / 数 % 程度が限界。
