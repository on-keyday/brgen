...
## バックエンドの完了条件: 1 本アプリを書く

これは EBM の失敗への対処ではなく、**フェーズの移行**である
(ユーザー談、2026-08-30)。

EBM で 18 言語ぶんの生成器を作ったのは、**各言語を最低限動かすには何が要るか
の実験**だった。その答えは出ている — 今日測った「デフォルトフック 103 本に
対し言語側は 3〜30 本 (wuffs 3 / java 6)、knob 設定は 50〜83 件」という数字も、
横断調査の「共通化不適 5 グループ」という否定的結論も、**多数の言語を並べた
から出た**もので、少数を深く作っていたら分からなかった。最低限の線は引けた。

だから次は線を引き直す。前フェーズの完了基準は「生成コードがコンパイルを
通り、期待通り動く」(= unictest) で、それは達成された。次フェーズの基準は
**「その言語の実プログラムから使える」**にする。前の基準が誤りだったのでは
なく、上げるという話。既存の ebm2* を遡って全部アプリを書く必要はない。

なぜ unictest では足りないか: unictest が確かめるのは **生成器が壊れて
いないこと**で、**その言語のプログラムに組み込めること**ではない。
ハーネスは入出力もビルドも全部こちらで用意するので、実利用でしか出ない
問題が表に出ない —

- 生成コードの API がその言語の慣習で呼べるか (命名、可視性、エラー型)
- 既存のバッファ / ストリーム / ソケットに繋がるか
- その言語の普通の依存の仕方 (module / crate / npm / Maven …) に乗るか
- 生成物の粒度が実用に耐えるか (1 ファイル巨大でないか)

実際、生成コードの見え方に関する判断 (ADR 0044 の生成器採番を public に
出さない、ADR 0027 の anon inner accessor 集約) が ebm2java / ebm2go で
先に出たのは、その 2 つが実利用に近かったからと読める。実利用が新しい
判断を引き出す、という関係がそこに見えている。

### 下限 (これを満たせば「1 本書いた」とする)

- 生成コードを **外から依存として取り込む**プログラムが 1 つ。生成物を
  コピペせず、その言語の普通の依存の仕方で参照する
- **自前の入出力**に繋いで動く (ファイル / ソケット / 既存バッファ)
- 題材は既存資産でよい。`example/` のフォーマット (pcap / zip / elf / png
  など) を読んで何か表示する CLI で足りる。規模は問わない

「本番運用に耐える」は求めない。そこまで求めると終わらない。

### 引き換えに失うもの

この規則は**言語を増やす速度を意図的に落とす**。ebm2wuffs が 3 フックで
立ったような「とりあえず出力が出た」段階で止められなくなる。
ただし前フェーズで「最低限に何が要るか」は測り終えているので、
速度を落とす代わりに得るものがある段階に来ている、という判断。
言語カバレッジの数を成果として見せたい場面とは衝突する。
