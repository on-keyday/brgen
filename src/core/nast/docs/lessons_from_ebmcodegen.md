...
を持っており、ebmcodegen とは**独立に同じ発想へ到達している部分が多い**。
差が出るのは lowering 層 (フック機構と共有 util) と、出力の作り方
(CodeWriter) の 2 つ。
...
nast の `access.h` も consteval + static_assert でパス検証しており、
ここは既に同じ土俵にいる。
...
パラメータ化する。nast の lowering が複数の出力先を持つようになったら
同じ形にする。
...
- **ADR 0003**: BM の最大の教訓 —「普通のコンパイラのように意味論を削って
  ...
  構造や意味を保ったまま渡す必要がある」。
  [[exit_and_reversibility]] と完全に同じ結論に、EBM 設計時点で到達していた
...
nast 側は `docs/` に議論のメモが 7 本あるだけで ADR 形式の判断記録は無い。
lowering に入ると判断が増えるので、同じ形式で残す価値がある。

## 活かせない / 既にやっているもの

- **フックの `__has_include` 解決**: 言語バックエンドが複数ある前提の機構。
  nast の lowering がまだ 1 つの出力先しか持たない間は過剰。ただし
  「置くだけで差し替わる」性質は将来効く
...
1. `unparse.cpp` を LocWriter ベースに移す (A)。インデント管理が構造化され、
   ついでに source map の下地が入る
