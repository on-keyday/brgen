...
EBM の反省 (ユーザー談、2026-08-30): **生成器は 18 言語ぶん作ったが、
生成コードが実アプリケーションに使われたのは Go など一部だけだった。**
nast のバックエンドは「作ったらそれで何か 1 つアプリを作る」を完了条件に
含める。
...
言語カバレッジの数を成果として見せたい場面とは衝突するが、
数より実効性を取る判断として意識的に選ぶ。
