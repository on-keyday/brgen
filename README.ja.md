...
最終的な目標(大それた野望とも言う)は、バイナリフォーマットの仕様書に brgen の定義言語を添えることが常識となることです。機械可読で曖昧さのない定義があれば、仕様書を手に入れて即コード生成ができます。生成コードが使えない場面でも、意味的に曖昧さのない定義書は手書き実装や初学者の理解に大いに役立つはずです。
