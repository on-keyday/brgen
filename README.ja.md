...
- **Protocol Buffers / Thrift / Cap'n Proto** — RPC/シリアライゼーション用の IDL。フォーマットを自分で新しく設計する用途のものであり、TCP ヘッダや TLS レコードといった既存のバイナリプロトコルの記述には使えません。一方 brgen は、その既存フォーマットの記述を主目的としています。
